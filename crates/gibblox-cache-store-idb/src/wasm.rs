...
impl IdbCacheStore {
    pub async fn open(
        ...
        let window = web_sys::window().ok_or_else(|| {
            GibbloxError::with_message(GibbloxErrorKind::Unsupported, "window not available")
        })?;
        let factory = window.indexed_db().map_err(js_io)?.ok_or_else(|| {
            GibbloxError::with_message(GibbloxErrorKind::Unsupported, "indexedDB unavailable")
        })?;
...
impl CacheStore for IdbCacheStore {
    ...
    async fn write_blocks(&self, start_block: u64, data: &[u8]) -> GibbloxResult<()> {
        ...
        let end = start_block.checked_add(blocks).ok_or_else(|| {
            GibbloxError::with_message(GibbloxErrorKind::OutOfRange, "block overflow")
        })?;
...
async fn validate_meta(db: &IdbDatabase, block_size: u32, total_blocks: u64) -> GibbloxResult<()> {
    let tx = db
    ...
    let stored_block_size = js_sys::Reflect::get(&value, &JsValue::from_str("block_size"))
        ...
        .as_f64()
        .ok_or_else(|| {
            GibbloxError::with_message(GibbloxErrorKind::Io, "indexeddb meta missing block_size")
        })? as u32;
    let stored_total_blocks = js_sys::Reflect::get(&value, &JsValue::from_str("total_blocks"))
        .map_err(js_io)?
        .as_f64()
        .ok_or_else(|| {
            GibbloxError::with_message(GibbloxErrorKind::Io, "indexeddb meta missing total_blocks")
        })? as u64;
...
fn js_io(err: impl Into<JsValue>) -> GibbloxError {
    GibbloxError::with_message(GibbloxErrorKind::Io, js_value_to_string(err.into()))
}
